...
def _base64_klartext(*texte):
    stuecke = []
    gesamt = 0
    for text in texte:
        if not text:
            continue
        for block in _B64_RE.findall(text)[:_B64_MAX_BLOECKE]:
...
def abgeleitete_texte(text):
    ...
    # Verpackung, an der beide Pruefungen vorbeiliefen.
    dekodiert = _base64_klartext(text, tag_text)
    if len(dekodiert) > 3:
